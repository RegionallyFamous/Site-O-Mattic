...
- WordPress can support the needed grammar with core blocks such as `Cover`, `Group`, `Columns`, `Media & Text`, `Image`, `Gallery`, `Navigation`, `Buttons`, `Table`, `Details`, `Quote`, `Pullquote`, `Spacer`, and `Site Logo`. The official core-block reference confirms these blocks and their supports.
...
- Navigation and actions: `Site Logo`, `Navigation`, `Buttons`, linked `Paragraph` / `List` when a nav block is too heavy.
...
| Side-rail service story | Sticky desktop rail contains logo, anchors, contact action, and one proof cue while the main content scrolls in strong bands. | Desktop side rail collapsing to compact top header on mobile. | Rail plus hero, proof, scope, process, objections, final contact. | Compact utility sans; strong section labels; body text stays readable and unfussy. | Rail can use dark/brand field; main content stays paper/ink; action remains consistent across rail and body. | Rail as the only persistent surface; main sections mostly flat. | `Group` shell, `Navigation`, `Site Logo`, `Buttons`, `Columns`, scoped CSS for rail. | Main content offset errors, rail consumes desktop width, duplicated mobile nav, z-index/focus issues. |
...
- Keep the first mobile viewport measurable: logo, H1/service promise, proof cue, primary CTA, and meaningful media should be visible at 360px, 390px, and 430px unless the silhouette intentionally trades one for a fixed mobile action dock.
