...
| `zone-grid-planner` | Garage organization, closet organization, pantry organization, mudroom organization | Split nav/action header with zone anchors | Split copy with garage photo and zone map | Zone plan cards with grid map note | Floor-first proof strip with zone metrics | Hero design-garage CTA plus final photo-plan quote |
| `urgent-checklist` | Gutter cleaning, holiday lights, vacation rental turnover, solar panel cleaning | Utility header with urgent CTA and short anchors | Service image with reserve-date board | Install/checklist cards with date-pressure proof | Seasonal safety proof strip before scope | Hero reserve-date CTA plus final quote |
| `service-receipt-stack` | Solar panel cleaning, window cleaning, detailing, vacation turnover | Compact top nav plus phone button | Media & Text hero with estimate receipt stack | Receipt cards plus readable scope table | Ledger proof strip and safety quote | Hero build-estimate CTA plus final instructions |
